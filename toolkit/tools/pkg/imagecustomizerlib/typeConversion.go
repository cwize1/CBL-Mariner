...
func partitionToImager(partition imagecustomizerapi.Partition, fileSystems []imagecustomizerapi.FileSystem,
	...
	imagerFlags, err := bootPartitionTypeToImager(partition.BootPartitionType)
	if err != nil {
...
func bootPartitionTypeToImager(bootType imagecustomizerapi.BootPartitionType) ([]configuration.PartitionFlag, error) {
	switch bootType {
	case imagecustomizerapi.BootPartitionTypeDefault:
		return nil, nil

	case imagecustomizerapi.BootPartitionTypeESP:
		return []configuration.PartitionFlag{configuration.PartitionFlagBoot, configuration.PartitionFlagESP}, nil
	...
	default:
		return nil, fmt.Errorf("unknown partition boot type (%s)", bootType)
	}
}
