...
func TestShouldFailParsingBadKernelCommandLine_SystemConfig(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "invalid [KernelCommandLine]: the 'ExtraCommandLine' field contains an invalid character", err.Error())

	err = remarshalJSON(badKernelCommandConfig, &checkedSystemConfig)
	assert.Error(t, err)
	assert.Equal(t, "failed to parse [SystemConfig]: failed to parse [KernelCommandLine]: the 'ExtraCommandLine' field contains an invalid character", err.Error())
}
